...
export default class FilesUpload {
    ...
    initFilesUpload(uploadFiles, context) {
        ...
        if (filesSize > self.inputFileMaxAllowedSize) {
            fileField.setCustomValidity(t('form.max_size_reached').replace('%fileSize%', this.humanFileSize(self.inputFileMaxAllowedSize, true)))
            self.setError(t('form.max_size_reached').replace('%fileSize%', this.humanFileSize(self.inputFileMaxAllowedSize, true)))
        } else {
        ...
        clearAllTag.innerHTML = t('form.remove_all')
        clearAllTag.href = '#'
        if(filenames.length === 0) {
           ...
           if (self.boxFileupload.querySelector('.remove-all-files') !== null ) {
            ...
            if (filenames.length === 1) {
                p.innerHTML = t('form.file_selected')
                if (self.boxFileupload.querySelector('.remove-all-files') !== null ) {
                    self.boxFileupload.querySelector('.remove-all-files').remove()
                }
            } else {
                p.innerHTML = t('form.files_selected').replace('%count%', filenames.length)
                if (self.boxFileupload.querySelector('.remove-all-files') == null ) {
            ...
            for (let i = 0; i < filenames.length; ++i) {
                ...
                if (uploadFiles.item(i).type !== undefined && self.acceptTypes && !self.acceptTypes.toLowerCase().includes(uploadFiles.item(i).type)) {
                    ...
                    span.innerHTML = t('form.format_not_supported')
                    li.prepend(span)
                    fileField.setCustomValidity(t('form.incorrect_format'))
                }
                const a = document.createElement('a')
                a.className = 'remove-file'
                a.innerHTML = t('form.remove')
                a.href = '#'
                a.dataset.fileid = i
